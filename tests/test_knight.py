...
class TestKnight(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.__board__.set_piece(4, 4, self.__knight_white__)  # Coloca el caballo blanco en e5
        self.__board__.set_piece(6, 5, self.__knight_black__)  # Coloca el caballo negro en f7
    ...
    def test_capture_opponent(self):
        ...
        self.__board__.set_piece(6, 5, self.__knight_black__)  # Coloca el caballo negro en f6
    ...
    def test_capture_own_piece(self):
        ...
        self.__board__.set_piece(6, 5, self.__knight_white__)  # Coloca otra pieza blanca en f6
        with self.assertRaises(InvalidPieceMove):
            self.__knight_white__.mov_correcto(4, 4, 6, 5)  # Movimiento no válido

    def test_invalid_move_shape(self):
        # Verifica que un movimiento válido en forma de L hacia una posición ocupada por una pieza del mismo color sea rechazado
        self.__board__.set_piece(3, 3, self.__knight_white__)  # Coloca otra pieza blanca en c4
        with self.assertRaises(InvalidPieceMove):
